...
public class App {
    ...
    public static MongoDatabase getConnect(String host, String user,
        ...
        MongoClientSettings settings = MongoClientSettings.builder()
                ...
                .applyToClusterSettings(builder -> builder
                        .hosts(Arrays.asList(new ServerAddress(host, 27017))))
                .build();
    ...
    private static void insertOneTest(MongoDatabase mongoDatabase,
            String collectionString) {
        // 获取集合
        MongoCollection<Document> collection = mongoDatabase
                .getCollection(collectionString);
        // 要插入的数据
        Document document = new Document("name", "张三").append("sex", "男")
                .append("age", 18);
        // 插入一个文档
        collection.insertOne(document);
    }

    public static long getCount(MongoDatabase mongoDatabase,
            String collectionString) {
        MongoCollection<Document> collection = mongoDatabase
                .getCollection(collectionString);
    ...
    public static Document findOne(MongoDatabase mongoDatabase,
            String collectionString) {
        MongoCollection<Document> collection = mongoDatabase
                .getCollection(collectionString);
    ...
    public static List<Document> findAll(MongoDatabase mongoDatabase,
            String collectionString) {
        MongoCollection<Document> collection = mongoDatabase
                .getCollection(collectionString);
    ...
    public static UpdateResult update(MongoDatabase mongoDatabase,
            String collectionString) {
        MongoCollection<Document> collection = mongoDatabase
                .getCollection(collectionString);
        return collection.updateOne(Filters.eq("name", "张三1"),
                new Document("$set", new Document("age", 40)));
    }

    public static DeleteResult delete(MongoDatabase mongoDatabase,
            String collectionString) {
        MongoCollection<Document> collection = mongoDatabase
                .getCollection(collectionString);
